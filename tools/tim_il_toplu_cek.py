"""741 il×sektör serisini tek koşuda üretir (bülten önbelleği paylaşımlı).

`ingest.run --only` seri başına ayrı süreç açar ve her seferinde ~44 XLSX
dosyasını yeniden indirir. Bu betik `il_seri_cek`i tek süreçte, tek
önbellekle çağırır: 741 seri için 44 indirme.
"""
...
def main() -> int:
    seriler = [s for s in serileri_yukle() if s.kaynak_tipi == "tim_il"]
    oturum = requests.Session()
    onbellek: dict = {}
    hatalar = []
    for sira, seri in enumerate(seriler, start=1):
        try:
            df = tim.il_seri_cek(seri, onbellek=onbellek, session=oturum)
